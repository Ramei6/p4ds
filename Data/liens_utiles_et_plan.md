...
Quel lien entre la densité du bati et transports/la densité du maillage à Paris ? https://github.com/Ramei6/p4ds
...
Cette base de donnée permet de géolocaliser chaque arrêts de bus, de métro, de tramway. On peut utiliser ArRName pour ne sélectionner qu’un arrêt par station et ArRGeopint pour les affilier à des zones géographiques.

A la frontière des transports en commun, on peut placer les réseaux de transports à usage individuel comme le taxi ou les stations de vélos en libre service. On eput présumer qu'au plus la densité de bâti est importante, au plus ces solutions sont proposées. Mais qu'en est-il vraiment.
...
Par ailleurs on peut se demander si les pouvoirs publics ont adapté ou non l'espace public à forte densité en donnat une plus grande part de la surface de stationnement aux moyens de transport individuels économes en place comme les motocylettes et byclettes et en enfouissant davantage de parkings dédiés à l'automobile. 
...
Egalement, nous allons étudier le lien entre la densité de bâti, du maillage routier et la présence d'arbres et d'espaces verts. Pour être plus précis, nous allons nous intéresser à la présence d'arbres dans les rues en fonction de la 
densité du maillage routier et donc au nombre d'arbres dans les rues au kilomètre carré.  Cette donnée permet de mesurer le degré d’artificialisation et donc mesurer une forme de qualité de vie. L’idée est de regarder alors le lien avec la densité du bâti et le type de maillage. (Est-ce que beaucoup de petites rues (probablement plus étroites) permettent de mettre plus ou moins d’arbre, et avec quel effet sur la densité du bâti ?)
...
Cette base de données répertorie et géolocalise tous les arbres de Paris. Contrairement aux autres bases de données de la ville de Paris concernant les arbres, elle semble bien exhaustive.
...
#On peut également s'intéresser au lien entre le niveau de vie INSEE et densité même si ce n'est pas le coeur de notre étude.

#On peut aussi se demander si l'on peut relier la densité du bâti avec l'âge des bâtiments. Malheureuseusement, les données de la base de données de Paris semblent défectueuses. En effet, le louvre y est daté de 2010, comme de nombreux bâtiments historique, et des immeubles connus de certains membres du groupe sont également fortement rajeûnis. Trouver des données de bonne qualité à ce sujet n'est pas chose aisée. Notre dernière espoir est la [BDNB de l'IGN :](https://bdnb.io/archives_data/bdnb_millesime_2025_07_a/)
Si cela fonctionne, on peut obtenir une carte explicite de l’age des bâtiments dans le tout Paris, et croisée l'âge moyen des bâtiments d'une zone géographique avec sa densité de bâti ou de maillage routier.
...
#En retranchant les espaces verts de grandes tailles, on obtient une bonne idée des densité de bâti local
[Iris]()
